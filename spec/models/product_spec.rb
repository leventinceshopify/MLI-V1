require 'rails_helper'

RSpec.describe Product, type: :model do
  it 'has a valid factory' do
    # Check that the factory we created is valid
    expect(build(:product)).to be_valid
  end

  let(:attributes) do
    # {
    #   name: 'Kodak DSLR2',
    #   description: 'Very good super machine'
    # }

      attrs = attributes_for(:product)
  end

let(:product) { create(:product, **attributes) }
  # let(:product) { create(:product) }

  describe 'model validations' do
    # check that the name field received the right values
    it { expect(product).to allow_value(attributes[:name]).for(:name) }
    # ensure that the title field is never empty
    it { expect(product).to validate_presence_of(:name) }
    # ensure that the title is unique for each todo list
    it { expect(product).to validate_uniqueness_of(:name)}

    it { expect(product).to allow_value(attributes[:description]).for(:description) }

    end

    describe 'model associations' do
   # ensure a todo list has many items
   it { expect(product).to have_many(:variants) }
   # it { expect(product).to have_many(:items) }
 end
end
